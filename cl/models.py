...
def cl_init(cls, config):
    ...
    cls.attn = AttentionLayer(config)
...
def cl_forward(cls,
    ...
    outputs = encoder(
        ...
        output_hidden_states=True if cls.model_args.pooler_type in ['cls', 'avg_top2', 'avg_first_last'] else False,
        return_dict=True,
    )
    ...
    source_pooler_output, target_outputs = cls.pooler(attention_mask, outputs)
    pooler_output = cls.attn(source_pooler_output, target_outputs)
...
def sentemb_forward(
    ...
    outputs = encoder(
        ...
        inputs_embeds=inputs_embeds,
        output_attentions=output_attentions,
        output_hidden_states=True if cls.pooler_type in ['cls', 'avg_top2', 'avg_first_last'] else False,
        return_dict=True,
    )

    source_pooler_output, target_pooler_output = cls.pooler(attention_mask, outputs)
    pooler_output = cls.attn(source_pooler_output, target_pooler_output)
    pooler_output = cls.mlp(pooler_output)
